...
$id_registro = $POST['id_registro'];
    ...
    if($_POST['registro'] == 'actualizar'){
        ...
        try{
            $stmt = $conn->prepare('UPDATE registrados SET nombre_registrado = ?, apellido_registrado = ?, email_registrado = ?, fecha_registro = ?, pases_articulos = ?, talleres_registrados = ?, regalo = ?, total_pagado = ?, pagado = 1 WHERE ID_Resitrado = ? ');
    ...
    if($_POST['registro'] == 'eliminar'){
        ...
        try{
            $stmt = $conn->prepare('DELETE FROM registrados WHERE ID_Resitrado = ? ');
